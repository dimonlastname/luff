## 4.2.2
* При включенном `Application.Debug` добавлен html тег `data-state-key` - имя стейта
* Фиск `ComboBox` в `luff-comp` - ошибка расположение списка при нехватке высоты экрана, а также если список короче, чем `listVisibleLinesCount`. 
* Фикс `PeriodPicker` в `luff-comp` - не отображался календарь при выборе языка, отличного от ру 

## 4.2.1
* Улучшен механизм `Dispose` компонентов. Добавлен связанный с компонентом `IContent.CreateState` — этот стейт будет разрушен после `Dispose` компонента, также в `SubState` и `AddOnChange` добавлен параметр компонента, при разрушении которого будет разрушен этот сабстейт или отменена подписка (в случае AddOnChange). Также добавлен явный колбек `AddDisposeCallback`
* Некоторые фиксы для работы с `permission` компонентов
* Обновлён `ComboBox` в `luff-comp`, теперь его поведение более очевидное и доступен поиск в случае кастомного `render` элемента списка;   
